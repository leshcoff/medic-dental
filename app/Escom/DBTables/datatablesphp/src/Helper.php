...
class Helper
{
    ...
    public static function theadFormatter($thead, $rowspan = 1)
    {
        ...
        foreach ($thead as $v) {
            ...
            $th = "<span>".(isset($v['title']) ? $v['title'] : '') . "</span>";
            if(isset($v['tooltip'])) {
                $th = "<span rel=\"tooltip\" data-placement=\"top\" title=\"" . ((isset($v['tooltip']) ? $v['tooltip'] : '')) . "\">" . (isset($v['title']) ? $v['title'] : '') . "</span>";
            }

            $attr = array_intersect_key($v, array_flip(['class', 'colspan', 'rowspan']));
            $html .= '<th'.Helper::mapAttributes($attr).'>'.($th).'</th>';
        }
        $html .= '</tr>';
