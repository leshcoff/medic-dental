...
/**
 * Server-side part inspired from
 *
...
class DataTable
{
    ...
    protected static $columnParams = [
        ...
        'tooltip',          // para que muestre el tooltip
        'title',
        'defaultContent',
        'width',
        'visible',
     ...
     * @return self
     */
    public function setTableDom($idDom)
    {
        $this->tableDom = $idDom;
    ...
    public function getJsInitParam()
    {
        return $this->jsInitParameters;
    }
    ...
    protected function setTHead(&$params)
    {
        ...
        if (isset($params['parent'])) {
            $this->theadChild[] = [
                ...
                'colspan'   => 1,
                'tooltip'   => isset($params['tooltip']) ? $params['tooltip'] : null,
            ];
            ...
            if (isset($params['tooltip'])) {
                $this->thead[$k]['tooltip']   = $params['tooltip'];
            }
        }
    }
    ...
    public function setColumn($params, $show = true)
    {
        if ($show) {
            ...
            #--------------------------------------------
            if(isset($params['tooltip'])){
                $params['title'] = "<span  data-uk-tooltip=\"{pos:'bottom', cls:'long-text'}\" title=\"" . ((isset($params['tooltip']) ? $params['tooltip'] : '')) . "\">".(isset($params['title']) ? $params['title'] : '') . "</span>";
            }
            #---------------------------------------------
    ...
    public function getJavascript()
    {
        ...
        $js = 'var '.$this->tableName.' = $("#'.$this->tableDom.'").DataTable('."\n"
            .(!empty($this->jsInitParameters) ? PHPToJS::render((object) $this->jsInitParameters, $this) : '')."\n"
            .');'."\n";
    ...
    protected function getJavascriptSearchPart()
    {
        ...
        // fix para que pueda buscar en el mismo orden de las columnas
        $js = $this->tableName .'.columns().every( function () {'.chr(13);
        $js.= '   var that = this;'.chr(13);

        $js.="    $( this.footer() ).on( 'keyup change blur', function (e) {".chr(13);
        $js.=        ($this->isServerSide() ? '    if ( e.keyCode == 13 ) {'.chr(13) : '');
        $js.="        if ( that.search() !== this.value ) {".chr(13);
        $js.="             $(this).attr('placeholder', this.value);".chr(13);
        $js.="             that".chr(13);
        $js.="             .search( ";
        $js.=               ($this->renderFilterOperators ? '( this.tagName != "INPUT" ||  this.value == \'\' ? \'\' : $(this).prev().find("select").val()) + this.value ' : ' this.value ');
        $js.="              )".chr(13);
        $js.="             .draw();".chr(13);
        $js.="        }".chr(13);
        $js.=        ($this->isServerSide() ? '    }'.chr(13) : '');
        $js.="    } );".chr(13);
        $js.="});".chr(13);


        //$js = 'var '.$this->tableName.'Api = '.$this->tableName.'.api(true);'.chr(13)
        /*$js = 'var '.$this->tableName.'Api = '.$this->tableName.';'.chr(13)
            .'$("#'.$this->tableDom.' tfoot th").each( function(colI) {'.chr(13)
                .'$(".sSearch", this).on("keyup change", function (e) {'.chr(13)
                .($this->isServerSide() ? '    if ( e.keyCode == 13 ) {'.chr(13) : '')
                .'        '.$this->tableName.'Api.column( colI ).search('.chr(13)
                .'            '.($this->renderFilterOperators ? '( this.tagName != "INPUT" ||  this.value == \'\' ? \'\' : $(this).prev().find("select").val()) + this.value ' : ' this.value ')
                .'        ).draw();'.chr(13)
                .($this->isServerSide() ? '    }'.chr(13) : '')
                .'});'."\n"
            .'});'."\n";
    ...
    public function getHtml($tAttributes = [])
    {
        ...
        $tAttributes['id'] = (isset($tAttributes['id']) ? ''.$tAttributes['id'] : $this->tableName);
    ...
    protected function renderFilter($column)
    {
        ...
        switch ($sFilter['type']) {
            case 'text'   :
                ...
                $r = '<div class="input-group-x"><div class="input-group-addon-x">'
            ...
            for ($j = 0, $jen = count($this->columns); $j<$jen; ++$j) {
                ...
                if (isset($column['formatter'])) {
                    if (isset($column['data']) || isset($column['sql_name'])) {
                        ...
                        $row[ isset($column['data']) ? $column['data'] : $j ] = call_user_func($column['formatter'], $data[$i]->{self::fromSQLColumn($column) }, get_object_vars($data[$i]), $column);
                    } else {
                        $row[ $j ] = call_user_func($column['formatter'], get_object_vars($data[$i]));
                    }
                } else {
        ...
        if (isset($this->request['order']) && count($this->request['order'])) {
            ...
            for ($i = 0, $ien = count($this->request['order']);$i<$ien;$i++) {
                ...
                if (!isset($column['orderable']) || $column['orderable'] === true) {
                    $orderBy[] = $this->toSQLColumn($column, 2).' '.(  strtoupper($this->request['order'][$i]['dir']) === 'ASC' ? 'ASC' : 'DESC');
                }
            }
    ...
    public function exec(array $request, $csv = false)
    {
        ...
        if ($this->counters) {
            try {
                ...
                $q = \DB::select($queries['recordsFiltered']);
            } catch (Exception $Exception) {
