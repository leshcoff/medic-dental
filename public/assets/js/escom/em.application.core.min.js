...
var App = function () {
    ...
    var fitModal = function(modal){
        var body, bodypaddings, header, headerheight, height, modalheight, modaldialog, modalpaddigns;
        var $window     = $(window);
        var limit       = 500;
    ...
    return {
            ...
            $.each(menu, function (idx, row) {
                ...
                if(type=='btn-group'){
                    ...
                    $.each(items, function(index, value){
                        ...
                        var btn    = $('<button id="'+config.id+'">').addClass('btn btn-responsive '+cls).addClass(size).append(icono);
                ...
                if(type=='dropdown'){
                    ...
                    if(showLabel){
                        btn.append('<span> '+ this.title+' &nbsp;</span><span class="caret"></span>');
                    }else{
        ...
        executeFunction : function(functionName, context) {
            ...
            if(functionName==null || functionName==''){
                alert ("--> No definiste o no existe la funcion a ejecutar");
                return false;
            }
		...
		*}
        ...
        handleResponse: function (responseText, status, line) {
            ...
            if (status != 200) {
                this.showError(status, "No se proceso la petición", responseText);
                return false;
            }
        ...
        verifyUser:function(options){
            //options {caller,interval,callback}
            //preguntamos si necesitamos solicitar la contraseña
            let url = App.getEmpresaUrl()+'/verify-user'
            let callback = options.callback;

            let data = {
                interval: (Number.isInteger(options.interval))? options.interval : 0,
                caller: (options.caller == undefined)? "" : options.caller
            };
            ...
            $.post(url,data,function(response){
                ...
                if(response.success == true){
                    ...
                    //si falla la verificacion hay que pedir contraseña
                    if(! response.request.transaction){
                        ...
                        let options = {
                            ...
                            typeInput: 'password',
                            btnCancel: true,
                            attributes: {id: 'VERIFY_USER_PASSWORD'},
                            successCallback: function(fieldText){
                                
                                //inyectamos el password en la peticion
                                data.password = fieldText;

                                $.post(url,data,function(response){

                                    if(response.success == true){
                                        if(response.request.transaction == false){
                                            EMAlerts.error({ 'message': response.request.errors.message});
                                        }else{
                                            //si la contraseña es correcta ejecutamos el callback
                        ...
                        EMAlerts.input(options);

                    }else{
                        //si estamos en periodo de gracia ejecutamos el callback
        ...
        getEmpresaUrl(){
            ...
            //sanitizamos el url y dividimos los segmentos
            let _url = window.location.href;
            let _base = window.location.protocol + '//' + window.location.host + '/';

            let _segments  = _url.replace(_base, "");
            _segments  = _segments.split('/');

            //obtenemos el indice de empresa para conservarlo junto con su hash
            let _index = _segments.indexOf("empresa");

            //eliminamos segmentos posteriores a la empresa y hash
            _segments.splice(_index + 2);

            //retornamos la url con el segmento de empresa y su hash
            return _base + _segments.join('/');
        }
...
jQuery.download = function(url, data, method){
    ...
    if( url && data ){
        ...
        //send request
        jQuery('<form target="_blank" action="'+ url +'" method="'+ (method||'post') +'">'+inputs+'</form>').appendTo('body').submit().remove();
    };
};
